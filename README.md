...
The included tests verify cross-agent isolation, database-backed duplicate-payment prevention, partial-payment balances and receipts, final-payment enforcement, and audit logging.
...
- For protection against server, disk, or database loss, secure the application database with infrastructure-level backups and restrict storage access.
...
3. On first boot the application creates all tables from the models (`db.create_all()`). The tables are portable: `agents`, `customers`, `payments`, `payment_receipts`, `audit_logs`, `refresh_tokens`.
...
Protect against server, disk, or database loss with infrastructure-level backups of the PostgreSQL database (for example, `pg_dump` on a schedule) and store the resulting files off the application server.
...
2. Deploy new code — `db.create_all()` is additive and never drops or resets tables.
3. After deploy, confirm agent/customer counts are unchanged.
